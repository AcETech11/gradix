...
export function TermSelector({ code, currentAcademicYear, currentTerm, options }: TermSelectorProps) {
  if (options.length <= 1) {
    return null;
  }
        ...
        {options.map((option) => {
          ...
          return (
            <Link
              ...
              href={`/results/${encodeURIComponent(code)}?term=${option.term}&year=${encodeURIComponent(option.academicYear)}`}
